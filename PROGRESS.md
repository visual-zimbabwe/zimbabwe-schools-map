...
- Removed local-only reports and CSV exports from Git tracking and ignored them.
- Added pytest-based data pipeline tests for cleaning and GeoJSON outputs.
- Reworked data pipeline tests to avoid pytest tmp_path due to permission errors in this environment.
- Adjusted data pipeline tests to use system temp files and avoid tmpdir permissions; tests pass with system Python + pytest (2 passed).
- Enforced allowed school levels and grant classes in cleaning script; invalid values are cleared and reported.
- Updated data pipeline tests to cover invalid levels/grants and recreated tests folder.
- Split missing lat/lon stats into raw and final counts with separate percentages in the cleaning report.
- Updated tests to assert raw vs final missing lat/lon report lines.
- Adjusted missing lat/lon report assertions to match raw=0 and final=2 for the fixture.
- Replaced multi-select trigger text nodes with explicit label spans to avoid brittle firstChild updates.
...
- Updated CODEBASE_CRITIC.md with a fresh codebase critique.
- Updated cleaning to attempt UTM conversion when only one of lat/lon is missing.
- Added CSV schema validation to cleaning and GeoJSON build scripts with regression tests.
- Normalized province/district filter keys at load time to make filtering robust to casing/whitespace.
- Wrapped map script in createMapApp factory to eliminate global mutable state.
...
- Restored CODEBASE_CRITIC.md and PROGRESS.md locally while keeping them removed from GitHub.
- Updated AGENTS.md to forbid pushing any files listed in .gitignore.
